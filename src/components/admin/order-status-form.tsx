...
export function OrderStatusForm({
  ...
  const field =
    "w-full rounded-[var(--radius-control)] border border-[var(--line)] bg-white px-3 py-2 text-sm outline-none focus:border-[var(--brand)]";
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <button
        type="submit"
        disabled={pending}
        className="w-full rounded-[var(--radius-control)] bg-[var(--brand)] px-4 py-2.5 text-sm font-medium text-white transition hover:bg-[var(--brand-2)] disabled:opacity-60"
      >
