    ...
    <>
      ...
      {local && (
        <Notice tone="warn" title="Đang chạy ở chế độ local (DATA_SOURCE=local)">
          Đơn hàng và liên hệ chỉ ghi ra console, chưa lưu vào cơ sở dữ liệu. Đặt{" "}
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <StatCard
          label="Bài viết"
          value={posts.total}
          hint={`${posts.fullyTranslated} bài dịch đủ 6 ngôn ngữ`}
          href="/admin/posts"
        />
        <StatCard
          label="Liên hệ"
          ...
          {contacts.recent.length === 0 ? (
            ...
            <ul className="space-y-4">
              {contacts.recent.map((contact) => (
                <li key={contact.id} className="border-b border-[var(--line)] pb-4 last:border-0 last:pb-0">
