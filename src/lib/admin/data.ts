...
import { NEWS_CATEGORIES, getPosts, type LegacyArticle } from "@/lib/legacy";
import { locales, type Locale } from "@/lib/i18n/config";
import { detectSourceLocale } from "@/lib/i18n/localized-content";
...
export type PostRow = LegacyArticle & {
  categories: string[];
  sourceLocale: Locale;
  translated: Locale[];
  missing: Locale[];
...
const cachedNews = newsCache as Record<string, Partial<Record<Locale, unknown>>>;

/** News lives in JSON, not the database — list it with its translation coverage. */
export function listPosts(): PostRow[] {
  const byCategory = new Map<string, string[]>();
  for (const category of NEWS_CATEGORIES) {
    for (const slug of getPosts(category.slug).map((p) => p.slug)) {
      byCategory.set(slug, [...(byCategory.get(slug) ?? []), category.label]);
    }
  }

  return getPosts().map((post) => {
    // The article's own language needs no translation — the cache covers the rest.
    const sourceLocale = detectSourceLocale(`${post.title}\n${post.content}`);
    const targets = locales.filter((locale) => locale !== sourceLocale);
    const entry = cachedNews[post.slug] ?? {};
    return {
      ...post,
      categories: byCategory.get(post.slug) ?? [],
      sourceLocale,
      translated: targets.filter((locale) => entry[locale]),
      missing: targets.filter((locale) => !entry[locale]),
    };
  });
}

/** Slugs already pushed into the Supabase `posts` table by `npm run posts:sync`. */
export async function listSyncedPostSlugs(): Promise<Set<string>> {
  const supabase = await getServiceClient();
  if (!supabase) return new Set();

  const { data, error } = await supabase.from("posts").select("slug");
  if (error) {
    console.error("[admin] listSyncedPostSlugs:", error);
    return new Set();
  }
...
export async function getDashboardData(): Promise<DashboardData> {
  const [products, posts] = await Promise.all([listAdminProducts(), listPosts()]);
  ...
  const postStats = {
    total: posts.length,
    fullyTranslated: posts.filter((p) => p.missing.length === 0).length,
  };
